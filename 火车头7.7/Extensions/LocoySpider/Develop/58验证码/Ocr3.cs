    ...
    class Ocr3
    {
        ...
        static Ocr3()
        {
            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    object obj = Resources.ResourceManager.GetObject("_" + i.ToString()+"_"+j.ToString(), Resources.Culture);
                    Bitmap bmp = (Bitmap)obj;
                    //先搞成黑色
                    Threshold2Value(bmp, 150);
                    List<Point> points = GetWordPoint(bmp);
                    match.Add(i.ToString()+"_"+j.ToString(), points);
                    codeSize.Add(i.ToString() + "_" + j.ToString(), CalculateSize(points));
                }
...
PixelFormat.Format8bppIndexed
            ...
            for (int i = 0; i < bmp.Width; i++)//X方向位移
            {
                for (int j = 0; j < bmp.Height; j++)//y方向位移
                {
                    ...
                    foreach (var code in codes)//每个字符
                    {
                        for (int k = 1; k >-1; k--)
                        {
                            string rcode = code + "_" + k.ToString();
                            if (i < bmp.Width - codeSize[rcode].Width &&
                          j < bmp.Height - codeSize[rcode].Height) //比较
                            {
                                if (ComparePixel(matrix, i, j, rcode))
                                {
                                    if (code == "3" &&   //3和8高度重合，如果3匹配到，则在拿8试试，如果成功，则说明是8
                                        ComparePixel(matrix, i + 1, j, "8_"+k.ToString()))
                                    {
                                        targetchar = "8";
                                        i++;
                                    }
                    ...
                    if (targetchar != null)
                    {
                       // if (targetchar == "4" || targetchar == "1"||targetchar=="2" )//4和1重合度太高，所以X+1
                        {
                            i++;
                        }
        ...
        static bool ComparePixel(int[,] matrix, int x, int y, string code)
        {
            ...
            float sorce = _a / _c;
            //if (code[0] == '1' && sorce < 0.98)
            //{
            //    return false;
            //}
            if (code[0] == '2' && sorce > 0.85)
            {
                return true;
            }
            if (sorce > 0.90)

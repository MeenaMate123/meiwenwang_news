...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	$firststr='<span>总：'.$num.' 页</span>';
	//上一页
	if($page<>1)
	{
		$toppage='<a href="'.$dolink.$add['dofile'].$type.'"  class="preon">'.$fun_r['startpage'].'</a>';
		$pagepr=$page-1;
		if($pagepr==1)
		{
	...
	if($page!=$totalpage)
	{
		...
		$lastpage='<a href="'.$lastpagelink.'" class="nexton">'.$fun_r['lastpage'].'</a>';
	}
